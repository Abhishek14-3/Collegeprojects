    ...
    def extract(self, text: str, candidates: List[str] = None) -> List[Tuple[str, float, int]]:
        ...
        for original_phrase, words in phrase_list:
            ...
            if phrase_norm not in raw_phrase_scores:
                ...
                if freq == 0:
                    freq = 1
                raw_phrase_scores[original_phrase] = (score, freq)
            else:
                curr_score, curr_freq = raw_phrase_scores[original_phrase]
                raw_phrase_scores[original_phrase] = (max(curr_score, score), curr_freq)
        ...
        results = [
            (phrase, round(score / max_raw, 4), freq)
            for phrase, (score, freq) in raw_phrase_scores.items()
        ]
